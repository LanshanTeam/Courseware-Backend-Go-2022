...
![call-chain.png](image%2Fcall-chain.png)
...
- 
![circuit_breaker_pattern.svg](image%2Fcircuit_breaker_pattern.svg)
...
- **计数限流算法**
  ...
  ![limit_count.png](image%2Flimit_count.png)

  可能存在的问题是：请求分布的不均衡，比如在时间周期1分钟的第1秒，就把1000次请求用完了，那么最后59秒都是空白的。也可能直到最后1秒才有大批量的流量涌入，造成系统的不稳定。

- **固定窗口限流算法（采样时间窗）**

  相对于上一个 计数限流，多了个时间窗口的概念，计数器每过一个时间窗口就重置，重新开始计算

  ![count-fixed-window.png](image%2Fcount-fixed-window.png)
...
- **滑动窗口限流（记录每个请求到达的时间点）**
  ...
  ![limit-slide-window.png](image%2Flimit-slide-window.png)
...
- **漏桶算法（漏斗池算法）**
  ...
  ![limit-leaky-bucket.jpg](image%2Flimit-leaky-bucket.jpg)
...
- **令牌桶算法（定速流入）**
  ...
  相对于漏洞的定速流程那种的匀速消费模式，令牌桶可以将积压的令牌一下子花掉，所以在应对流量洪峰的时候，他的表现比漏桶算法更优异。

  ![limit-token-bucket.jpg](image%2Flimit-token-bucket.jpg)
...
![sentinel.png](image%2Fsentinel.png)
